...
export default function HomeScreen() {
  ...
  useEffect(() => {
    if (prevIsLoggedInRef.current !== isLoggedIn) {
      ...
      // 登录状态变化时，如果当前分类不是容器分类（需要选子标签的），刷新数据
      if (selectedCategory && apiConfigStatus.isConfigured && !apiConfigStatus.needsConfiguration) {
        if (!selectedCategory.tags || selectedCategory.tag) {
          fetchInitialData();
        }
      }
    }
  }, [isLoggedIn, selectedCategory, apiConfigStatus.isConfigured, apiConfigStatus.needsConfiguration, fetchInitialData]);
