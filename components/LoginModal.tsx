...
const LoginModal = () => {
  const { isLoginModalVisible, hideLoginModal, checkLoginStatus, markAsLoggedIn } = useAuthStore();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 直接标记为已登录，不再依赖 checkLoginStatus（已手动登录成功）
      markAsLoggedIn();
